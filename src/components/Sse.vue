...
const connectToSSE = () => {
  ...
  eventSource = new EventSource(`http://localhost:9023/sse/${connectionId.value}`)
...
const sendMessage = () => {
  ...
  fetch(
    `http://localhost:9023/sse?connectionId=${connectionId.value}&message=${encodeURIComponent(message.value)}`,
    {
      method: 'POST',
    },
  )
